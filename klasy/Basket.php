...
class Basket {
    ...
    foreach ($_SESSION['basket'] as $item) {
        ...
        $cena = $item->cena;
        $ile = $item->ile;
        $query = "INSERT INTO KsiazkiZamowienia VALUES($id, $orderId, $ile, $cena)";
    if (!$this->dbo->query($query)) // Jeśli nie udało się wykonać zapytania
        return SERVER_ERROR;
    if ($this->dbo->affected_rows <> 1) // Jeśli liczba dodanych rekordów inna niż 1
        return SERVER_ERROR;
    }
